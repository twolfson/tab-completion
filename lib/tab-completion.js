  ...
  install: function (cb) {
    ...
    async.waterfall([
      ...
      function getRcFiles (callback) {
        that._findExistingRcs(callback);
      },
      function handleRcFiles (_filepaths, callback) {
        // If there are no rcFilepaths, callback early
        rcFilepaths = _filepaths;
      ...
      function writeCompletionScript (completionScript, callback) {
        ...
        destFilepath = path.join(TabCompletion.completionDirectory, that.name);
        that._writeCompletionScript(destFilepath, completionScript, callback);
      },
